...
class TimeAnalyzer:
    ...
    def evaluate(charges_with_summary: ChargesWithSummary):
        if charges_with_summary.most_recent_conviction:
            elig_date = TimeAnalyzer._calc_elig_date(
                charges_with_summary.most_recent_conviction, TimeAnalyzer.TEN_YEARS
            )
            TimeAnalyzer._mark_as_time_ineligible(
            ...
            if len(recent_violation_convictions) == 0:
                ...
                if older_than_three_years:
                    ...
                    recent_violation_conviction.set_time_ineligible(
                        "Time-ineligible under 137.225(1)(a)", date_eligible
                    )
            else:
                raise ValueError(
                    "There is no MRC and no MRD, so the recent known charges should only contain a single violation or nothing."
                )
    ...
    def _calc_furthest_out_elig_date(charges_with_summary: ChargesWithSummary):
        if charges_with_summary.second_most_recent_conviction:
            date_1 = TimeAnalyzer._calc_elig_date(
                charges_with_summary.second_most_recent_conviction, TimeAnalyzer.TEN_YEARS
            )
            ...
            if class_b_felony.convicted():
                if TimeAnalyzer._calculate_has_subsequent_charge(class_b_felony, charges_with_summary.charges):
                    ...
                    if eligibility_date > date.today():
                        class_b_felony.set_time_ineligible(
                            "137.225(5)(a)(A)(i) - Twenty years from class B felony conviction", eligibility_date
                        )
                ...
                for charge in mrc.case()().charges:
                    if charge.acquitted():
                            charge.set_time_ineligible(
                                ...
                                second_mrc.disposition.date # type: ignore
