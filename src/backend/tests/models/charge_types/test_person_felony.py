...
def test_person_felony_class_b(person_felony_statute):
    ...
    assert any([isinstance(charge.charge_type, PersonFelonyClassB) for charge in charges])
...
def test_felony_b_person_felony_with_missing_subsection(person_felony_statute):
    ...
    assert isinstance(person_felony_class_b_convicted.charge_type, PersonFelonyClassB)
...
def test_other_charge_type_true_negatives(person_felony_statute_with_other_charge_type):
    ...
    assert [not isinstance(charge.charge_type, PersonFelonyClassB) for charge in charges]
...
def test_felony_not_b_person_felony(person_felony_statute_not_b_felony):
    ...
    assert not isinstance(person_felony_not_class_b_convicted.charge_type, PersonFelonyClassB)
