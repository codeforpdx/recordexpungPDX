...
function Questions(props: Props) {
  function select(ambiguous_charge_id: string, case_number: string) {
    return (question_id: string, answer: string, edit: any, date: string = "", probation_revoked_date: string = "") => {
      return selectAnswer(ambiguous_charge_id, case_number, question_id, answer, edit, date, probation_revoked_date);
    }
  }
  ...
  const render = () => {
    if (props.question_summary) {
      ...
      const selectFunction = select(props.question_summary.ambiguous_charge_id, props.question_summary.case_number);
      return (
  ...
  const renderQuestions = (question: QuestionData, selectFunction: Function) => {
    return (
  ...
  const renderQuestion = (question: QuestionData, selectFunction: Function) => {
    if (question.text === "Choose the disposition") {
      return (<DispositionQuestion question={question} selectFunction={selectFunction} />);
    } else {
      return (<Question question={question} selectFunction={selectFunction} />);
    }
  };

  function renderChildrenQuestions(question: QuestionData, selectFunction: Function): any {
...
function mapStateToProps(state: AppState, ownProps: Props) {
  if (state.search.questions && state.search.questions[ownProps.ambiguous_charge_id]) {
    const question_summary = state.search.questions[ownProps.ambiguous_charge_id];
    return {
